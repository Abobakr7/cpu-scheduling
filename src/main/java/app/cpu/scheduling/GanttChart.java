...
public class GanttChart extends JFrame {
    ...
    private ArrayList<String> prcsColor;
    
    public GanttChart(String title, int nProcess, double averageTAT, double averageWT, boolean isPreemptive, ArrayList<ProcessClass> processes) {
    ...
    private ChartPanel getChart() {
        ...
        for (int i = 0; i < this.nProcess; ++i) {
            renderer.setSeriesPaint(i, this.colors.get(this.prcsColor.get(i)));
        }
    ...
    private IntervalCategoryDataset getNonPreemptiveDataset() {
        ...
        for (int i = 0; i < this.processes.size(); ++i) {
            this.prcsColor.add(this.processes.get(i).getColor());
            TaskSeries s = new TaskSeries(this.processes.get(i).getName());
            s.add(new Task(this.processes.get(i).getName(),
                        new SimpleTimePeriod(this.processes.get(i).getStartTime(), this.processes.get(i).getCompelationTime())));
            dataset.add(s);
        }
        return dataset;
    }
    ...
    private IntervalCategoryDataset getPreemptiveDataset() {
        ArrayList<ArrayList<ProcessClass>> prcs = new ArrayList<>();
        for (int i = 0; i < this.nProcess; ++i) {
            prcs.add(new ArrayList<>());
        }
        for (int i = 0; i < this.processes.size(); ++i) {
            int index = this.processes.get(i).getOrder();
            prcs.get(index).add(this.processes.get(i));
        }

        TaskSeriesCollection dataset = new TaskSeriesCollection();
        for (int i = 0; i < this.nProcess; ++i) {
            this.prcsColor.add(prcs.get(i).get(0).getColor());
            TaskSeries ts = new TaskSeries(prcs.get(i).get(0).getName());
            int last = prcs.get(i).size() - 1;
            Task t = new Task(prcs.get(i).get(0).getName(),
                            new SimpleTimePeriod(prcs.get(i).get(0).getStartTime(), prcs.get(i).get(last).getCompelationTime()));
            for (int j = 0; j < prcs.get(i).size(); ++j) {
                Task tk = new Task(prcs.get(i).get(j).getName(),
                                new SimpleTimePeriod(prcs.get(i).get(j).getStartTime(), prcs.get(i).get(j).getCompelationTime()));
                t.addSubtask(tk);
            }
            ts.add(t);
            dataset.add(ts);
